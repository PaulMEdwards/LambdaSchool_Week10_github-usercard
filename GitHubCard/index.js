...
const Hme = document.createElement('h2');
Hme.textContent = "Me";
Hme.classList.add('me');
cards.appendChild(Hme);
const me = document.querySelector('h2.me');

const Hfollowers = document.createElement('h2');
Hfollowers.textContent = "Followers";
Hfollowers.classList.add('followers');
cards.appendChild(Hfollowers);
const followers = document.querySelector('h2.followers');

// const Hfollowing = document.createElement('h2');
// Hfollowing.textContent = "Following";
// Hfollowing.classList.add('following');
// cards.appendChild(Hfollowing);
// const following = document.querySelector('h2.following');


// Comment this out to use live API data
// const mockGitHubData = gitHubData();
const mockFollowers = followersData();
// const mockFollowing = followingData();

// Fetch data
const myGitHubPromise = typeof mockGitHubData !== "undefined" ? mockGitHubData : axios.get('https://api.github.com/users/PaulMEdwards');
console.log("myGitHubPromise:");
console.log(myGitHubPromise);
const followersArray = typeof mockFollowers !== "undefined" ? mockFollowers : axios.get('https://api.github.com/users/PaulMEdwards/followers').then((results) => {return results.data;});
console.log("followersArray:");
console.log(followersArray);
// const followingArray = typeof mockFollowing !== "undefined" ? mockFollowing : axios.get('https://api.github.com/users/PaulMEdwards/following');
// console.log("followingArray:");
// console.log(followingArray);

// My GitHub data
if (myGitHubPromise.then != undefined) {
  console.log('Promise path.');
  myGitHubPromise.then(function (results) {
    console.log("Results:");
    console.log(results);
    me.insertAdjacentElement("afterend", buildGitHubProfileCard(results.data));
  });
  myGitHubPromise.catch(function (error) {
    console.log("Error:");
    console.log(error);
  });
} else {
  console.log('Mock data path.');
  console.log(myGitHubPromise.results);
  me.insertAdjacentElement("afterend", buildGitHubProfileCard(myGitHubPromise.results.data));
}

// Followers
appendRandomGroupiesCards(followers, followersArray, 5);

// // Following
// appendRandomGroupiesCards(following, followingArray, 5);
...
function appendRandomGroupiesCards(target, data, count) {
  console.log("Data:");
  console.log(data);

  // for (let i = count-1; i >= 0; i--) {
  //   console.log(i);
  //   axios.get('https://api.github.com/users/'+data.then((d) => {return d[i].login;})).then((r) => {
  //     target.insertAdjacentElement("afterend", buildGitHubProfileCard(r.data));
  //   });
  // }
  ...
  for (let i = count-1; i >= 0; i--) {
    console.log(i);
    subSet.push(data[random(0, data.length-1)]);
  }
  console.log("subSet:");
  console.log(subSet);
  subSet.forEach(g => {
